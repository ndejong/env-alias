...
def test_sample_local_yaml_01(capsys):
    ...
    local_test_filename = os.path.join(os.path.dirname(__file__), 'samples', 'local.json')
    temp_configuration_file = __rewrite_configuration_file(
        os.path.join(os.path.dirname(__file__), 'configs', 'local_json_01.yml'),
    ...
    assert captured == ' export "local_json_01"="value02"'
...
def test_sample_local_yaml_02(capsys):
    ...
    local_test_filename = os.path.join(os.path.dirname(__file__), 'samples', 'local.json')
    temp_configuration_file = __rewrite_configuration_file(
        os.path.join(os.path.dirname(__file__), 'configs', 'local_json_02.yml'),
        source=local_test_filename
    )
    ...
    captured = capsys.readouterr().out.rstrip()
    assert captured == ' export "local_json_02"="value03"'
